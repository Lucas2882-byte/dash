...
def _auto_sync_github():
    ...
    try:
        import github_sync
        github_sync.upload_database_to_github(DATABASE_FILE)
    except:
        pass
...
def add_task(title, description, assigned_to, priority='Normale', client_name='', deadline=None):
    ...
    cursor.execute('''
        INSERT INTO tasks (title, description, client_name, assigned_to, priority, status, task_order, deadline)
        VALUES (?, ?, ?, ?, ?, 'À faire', ?, ?)
    ''', (title, description, client_name, assigned_to, priority, next_order, deadline))
...
def create_workflow_steps_for_listing(listing_id):
...
def ensure_all_listings_have_workflows():
    ...
    for listing in listings:
        create_workflow_steps_for_listing(listing[0])
...
def create_workflow_steps_for_service(service_id):
...
def ensure_all_services_have_workflows():
    ...
    for service in services:
        create_workflow_steps_for_service(service[0])
