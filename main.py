...
def strip_emoji_from_value(value):
    ...
    # Retirer tous les emojis utilisés dans l'application
    # Emojis pour les priorités et statuts
    emojis_to_remove = ['🔴 ', '🟢 ', '🟡 ', '💬 ', '⚙️ ', '👤 ', '📦 ', '💰 ', '📝 ', '📍 ', '🔍 ', '📞 ', '🚨 ', '⚪ ']
...
def render_tasks_table_with_edit(display_df, original_df, column_order, person_categories=None, editor_key="tasks_editor"):
    ...
    try:
        ...
        st.caption("⚠️ GitHub sync non configuré")
...
def show_workflow_dialog(listing_id):
    ...
    # Bouton pour enregistrer sur GitHub
    try:
        import github_sync
        from datetime import datetime
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📤 Enregistrer sur GitHub", key="save_workflow_github", use_container_width=True, type="primary"):
                with st.spinner("Sauvegarde en cours..."):
                    success, message = github_sync.upload_database_to_github(db.DATABASE_FILE)
                    if success:
                        st.session_state.last_github_upload = datetime.now().timestamp()
...
def show_service_workflow_dialog(service_id):
    ...
    try:
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📤 Enregistrer sur GitHub", key="save_service_github", use_container_width=True, type="primary"):
                with st.spinner("Sauvegarde en cours..."):
                    success, message = github_sync.upload_database_to_github(db.DATABASE_FILE)
                    if success:
                        st.session_state.last_github_upload = datetime.now().timestamp()
        ...
        with st.form("new_task_form", clear_on_submit=True):
            ...
            if submitted:
                if title and assigned_to:
                    ...
                    # Enregistrer automatiquement sur GitHub
                    try:
                        import github_sync
                        from datetime import datetime
                        success, message = github_sync.upload_database_to_github(db.DATABASE_FILE)
                        if success:
                            st.session_state.last_github_upload = datetime.now().timestamp()
                            st.success(f"✅ Tâche ajoutée et sauvegardée sur GitHub !")
                        else:
                            st.success(f"✅ Tâche ajoutée (local) - pensez à sauvegarder sur GitHub")
                    except:
                        ...
                        if business_name and client_name:
                            ...
                            try:
                                ...
                                if success:
                                    st.session_state.last_github_upload = datetime.now().timestamp()
                                    if create_local_service:
                                        st.success(f"✅ Fiche Google et Service Local ajoutés et sauvegardés sur GitHub !")
                                    else:
                                        st.success(f"✅ Fiche Google ajoutée et sauvegardée sur GitHub !")
                                else:
                                    st.success(f"✅ Fiche ajoutée (local) - pensez à sauvegarder sur GitHub")
                            except:
                                if create_local_service:
                    ...
                    if submitted_service:
                        if service_name:
                            ...
                            try:
                                ...
                                if success:
                                    st.session_state.last_github_upload = datetime.now().timestamp()
                                    st.success(f"✅ Service local ajouté et sauvegardé sur GitHub !")
                                else:
                                    st.success(f"✅ Service local ajouté (local) - pensez à sauvegarder sur GitHub")
                            except:
...
with st.sidebar:
    ...
    try:
        ...
        st.markdown("""
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                ...
                <p style="color: rgba(255,255,255,0.9); margin: 0; font-size: 13px;">
                    Synchronisez manuellement vos données
                </p>
            </div>
        """, unsafe_allow_html=True)
        
        if os.path.exists(db.DATABASE_FILE):
            db_modified_time = os.path.getmtime(db.DATABASE_FILE)
            db_modified_datetime = datetime.fromtimestamp(db_modified_time)
            
            last_upload_time = st.session_state.get('last_github_upload', None)
            
            if last_upload_time is None or db_modified_time > last_upload_time:
                st.markdown("""
                    <div style="background-color: #fff3cd; 
                        ...
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <div>
                                <strong style="color: #856404;">Changements non sauvegardés</strong>
                ...
                st.markdown("""
                    <div style="background-color: #d4edda; 
                        ...
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <div>
                                <strong style="color: #155724;">Tout est sauvegardé</strong>
                                <p style="margin: 4px 0 0 0; font-size: 12px; color: #155724;">
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📤 Upload", use_container_width=True, help="Sauvegarder la BDD sur GitHub", type="primary"):
        ...
        with col2:
            if st.button("📥 Download", use_container_width=True, help="Restaurer la BDD depuis GitHub"):
        ...
        st.caption("💡 Utilisez Upload pour sauvegarder vos changements")
    except Exception as e:
        st.warning("⚠️ Synchronisation GitHub non configurée")
