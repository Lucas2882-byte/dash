    ...
    with tab3:
        ...
        if person_tasks.empty:
            ...
            if filtered_person_tasks.empty:
                ...
                if has_active_filters and view_mode == "🎯 Drag & Drop":
                    st.warning("⚠️ La réorganisation des tâches n'est pas disponible lorsque des filtres sont actifs. Affichez toutes les tâches pour pouvoir réorganiser.")
                
                # Ajouter les numéros de tâche par personne
                filtered_person_tasks = add_person_task_numbers(filtered_person_tasks)
                
                if view_mode == "🎯 Drag & Drop" and not has_active_filters:
                    st.info("💡 Glissez-déposez les tâches pour les réorganiser, puis cliquez sur 'Enregistrer l'ordre'")
                    
                    # Préparer les items pour le drag-and-drop
                    ...
                    for idx, task in sorted_tasks.iterrows():
                        ...
                        client_text = f" - {task['client_name']}" if task['client_name'] else ""
                        task_id = int(task['_original_id'])
                        task_number = int(task['person_task_number'])
                        label = f"#{task_number} {priority_icon} {status_icon} {task['title']}{client_text}"
                        task_items.append(label)
                        task_id_map[label] = task_id
                    
                    # Widget de drag-and-drop
                    ...
                    if st.button("💾 Enregistrer l'ordre", use_container_width=True, type="primary"):
                        new_order_map = {}
                        for new_idx, item_label in enumerate(sorted_items):
                            task_id = task_id_map[item_label]
