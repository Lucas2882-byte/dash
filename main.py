        ...
        if person_tasks.empty:
            ...
            if filtered_person_tasks.empty:
                ...
                if view_mode == "🔀 Réorganiser" and not has_active_filters:
                    ...
                    # Créer une liste d'items avec les informations de chaque tâche
                    task_items = []
                    task_id_map = {}
                    ...
                    for idx, task in sorted_tasks.iterrows():
                        ...
                        # Créer un identifiant unique pour l'item
                        item_label = f"{priority_icon} {status_icon} {task['title']}{client_text}"
                        task_items.append(item_label)
                        task_id_map[item_label] = int(task['_original_id'])
                    ...
                    sorted_items = sort_items(task_items, multi_containers=False, direction='vertical')
                    
                    # Stocker l'ordre dans session_state
                    st.session_state.reorder_tasks_map = {task_id_map[item]: idx + 1 for idx, item in enumerate(sorted_items)}
