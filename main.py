...
try:
    ...
    if 'edit_task' in query_params:
        edit_task_id = query_params['edit_task']
        try:
            st.session_state.edit_task_id = int(edit_task_id)
            # Nettoyer le query param
            del st.query_params['edit_task']
        except (ValueError, TypeError):
            pass
    
    # Gérer la mise à jour de priorité
    if 'update_priority' in query_params and 'new_priority' in query_params:
        task_id = query_params['update_priority']
        new_priority = query_params['new_priority']
        try:
            task_id = int(task_id)
            ...
            del st.query_params['update_priority']
            del st.query_params['new_priority']
            st.rerun()
        except (ValueError, TypeError):
            pass
except Exception:
...
# Catégories de tâches pour chaque membre de l'équipe
team_categories = {
    "Franck": ["RETOUR CLIENT", "INTERNE", "NEW CLIENT", "COMMANDES", "RELANCE"],
    "Lise": ["GESTION FICHE", "LOCAL SERVICES"],
    "Lucas": ["PROSPECT", "RETOUR CLIENT", "RELANCE APPELS", "RELANCE COMMANDES", "INTERNE", "INFO URGENTE"]
}

# Map des emojis pour les catégories
CATEGORY_EMOJI_MAP = {
    "RETOUR CLIENT": "💬",
    "INTERNE": "⚙️",
    "NEW CLIENT": "👤",
...
def auto_save_to_github():
    ...
    try:
        ...
        # Garantir la libération du verrou même en cas d'exception
...
def add_emoji_to_category(category):
    """Ajoute l'emoji correspondant à une catégorie"""
    if not category or pd.isna(category):
        return category
    emoji = CATEGORY_EMOJI_MAP.get(category, '⚪')
    return f"{emoji} {category}"

def get_category_options_with_emojis(categories):
    """Retourne une liste de catégories avec emojis pour les selectbox"""
    return [add_emoji_to_category(cat) for cat in categories]
...
def strip_emoji_from_value(value):
    """Retire les emojis d'une valeur pour permettre le filtrage correct"""
    if pd.isna(value) or value == "":
        return value
    
    value_str = str(value)
    
    # Retirer tous les emojis utilisés dans l'application (avec ET sans espace)
    # Liste des emojis utilisés
    emoji_chars = ['🔴', '🟢', '🟡', '💬', '⚙️', '👤', '📦', '💰', '📝', '📍', '🔍', '📞', '🚨', '⚪']
    
    for emoji in emoji_chars:
        # Retirer l'emoji avec espace après
        value_str = value_str.replace(emoji + ' ', '')
        # Retirer l'emoji sans espace (pour les anciennes données)
        value_str = value_str.replace(emoji, '')
    
    return value_str.strip()

def create_status_tag(status):
    """Crée un tag HTML coloré pour le statut"""
    if pd.isna(status) or status == "":
        return ""
    
    color_map = {
        "À faire": ("#EF4444", "#FEE2E2"),
        "En cours": ("#F59E0B", "#FEF3C7"),
        "Terminée": ("#10B981", "#D1FAE5")
    }
    
    bg_color, text_color = color_map.get(status, ("#6B7280", "#E5E7EB"))
    
    return f'<span style="display: inline-block; padding: 4px 10px; border-radius: 8px; background: {bg_color}; color: white; font-size: 12px; font-weight: 600; white-space: nowrap;">{escape_html(status)}</span>'

def create_priority_tag(priority):
    """Crée un tag HTML coloré pour la priorité"""
    if pd.isna(priority) or priority == "":
        return ""
    
    # Nettoyer les émojis si présents
    clean_priority = priority.replace("🔴 ", "").replace("🟢 ", "").strip()
    
    color_map = {
        "Normale": ("#10B981", "#D1FAE5"),
        "Urgente": ("#EF4444", "#FEE2E2")
    }
    
    bg_color, text_color = color_map.get(clean_priority, ("#6B7280", "#E5E7EB"))
    
    return f'<span style="display: inline-block; padding: 4px 10px; border-radius: 8px; background: {bg_color}; color: white; font-size: 12px; font-weight: 600; white-space: nowrap;">{escape_html(clean_priority)}</span>'

def create_category_tag(category):
    """Crée un tag HTML coloré pour la catégorie"""
    if pd.isna(category) or category == "":
        return '<span style="display: inline-block; padding: 4px 10px; border-radius: 8px; background: #6B7280; color: white; font-size: 12px; font-weight: 600; white-space: nowrap;">Sans catégorie</span>'
    
    color_map = {
        "RETOUR CLIENT": "#8B5CF6",
        "INTERNE": "#6366F1",
        "NEW CLIENT": "#14B8A6",
        "COMMANDES": "#F59E0B",
        "RELANCE": "#F97316",
        "GESTION FICHE": "#8B5CF6",
        "LOCAL SERVICES": "#06B6D4",
        "PROSPECT": "#84CC16",
        "RELANCE APPELS": "#F43F5E",
        "RELANCE COMMANDES": "#FB923C",
        "INFO URGENTE": "#DC2626"
    }
    
    bg_color = color_map.get(category, "#6B7280")
    
    return f'<span style="display: inline-block; padding: 4px 10px; border-radius: 8px; background: {bg_color}; color: white; font-size: 12px; font-weight: 600; white-space: nowrap;">{escape_html(category)}</span>'

def render_tasks_table_with_edit(display_df, original_df, column_order, person_categories=None, editor_key="tasks_editor"):
    """Affiche un tableau Streamlit éditable directement
    
    Args:
        display_df: DataFrame à afficher
        original_df: DataFrame original avec les données brutes
        column_order: Liste des colonnes à afficher
        person_categories: Liste des catégories disponibles pour la personne (optionnel)
                          Si None, affiche toutes les catégories
        editor_key: Clé unique pour le data_editor (par défaut: "tasks_editor")
    """
    
    # Préparer le DataFrame pour l'affichage et l'édition
    df_to_edit = display_df[column_order].copy()
    
    # Ajouter des émojis colorés aux valeurs pour un affichage visuel
    if 'Priorité' in df_to_edit.columns:
        df_to_edit['Priorité'] = df_to_edit['Priorité'].apply(
            lambda x: f"🔴 {x}" if x == "Urgente" else f"🟢 {x}" if x == "Normale" else x
        )
    
    if 'Statut' in df_to_edit.columns:
        df_to_edit['Statut'] = df_to_edit['Statut'].apply(
            lambda x: f"🔴 {x}" if x == "À faire" else f"🟡 {x}" if x == "En cours" else f"🟢 {x}" if x == "Terminée" else x
        )
    
    # Ajouter des émojis pour les catégories
    if 'Catégorie' in df_to_edit.columns:
        df_to_edit['Catégorie'] = df_to_edit['Catégorie'].apply(
            lambda x: f"{CATEGORY_EMOJI_MAP.get(x, '⚪')} {x}" if pd.notna(x) else x
        )
    
    # Stocker les IDs originaux pour la mise à jour
    task_ids_map = {}
    for idx, row in display_df.iterrows():
        task_id = original_df.loc[idx, '_original_id'] if idx in original_df.index and '_original_id' in original_df.columns else (original_df.loc[idx, 'id'] if idx in original_df.index else idx)
        task_ids_map[idx] = task_id
    
    # Préparer les options de catégorie avec emojis
    if person_categories is not None:
        # Utiliser uniquement les catégories de la personne
        category_options_list = get_category_options_with_emojis(person_categories)
    else:
        # Toutes les catégories disponibles
        all_categories = ["RETOUR CLIENT", "INTERNE", "NEW CLIENT", "COMMANDES", "RELANCE", 
                         "GESTION FICHE", "LOCAL SERVICES", "PROSPECT", "RELANCE APPELS", 
                         "RELANCE COMMANDES", "INFO URGENTE"]
        category_options_list = get_category_options_with_emojis(all_categories)
    
    # Afficher le tableau éditable avec data_editor
    edited_df = st.data_editor(
        ...
        num_rows="fixed",
        column_config={
            "Ordre": st.column_config.NumberColumn("Ordre", width=50, disabled=True),
            ...
            "Titre": st.column_config.TextColumn("Titre", width=200, required=True),
            "Description": st.column_config.TextColumn("Description", width=1000),
            "Client": st.column_config.TextColumn("Client", width=150),
            "Catégorie": st.column_config.SelectboxColumn(
                "Catégorie",
                width=150,
                options=category_options_list
            ),
            "Date d'échéance": st.column_config.DateColumn("Date d'échéance", width=150)
        },
        key=editor_key
    )
    ...
    if not df_to_edit.equals(edited_df):
        ...
        # Créer un placeholder pour afficher les messages de progression
        status_placeholder = st.empty()
        status_placeholder.info("⏳ Sauvegarde automatique en cours...", icon="💾")
        ...
        # Tracker les modifications pour le message de confirmation
        modifications = []
        ...
        for idx in edited_df_pending.index:
            ...
            if task_id:
                ...
                original_row = df_to_edit.loc[idx] if idx in df_to_edit.index else None
                
                # Détecter les colonnes modifiées
                if original_row is not None:
                    changed_fields = []
                    for col in edited_df_pending.columns:
                        old_val = str(original_row.get(col, '')) if pd.notna(original_row.get(col)) else ''
                        new_val = str(edited_row.get(col, '')) if pd.notna(edited_row.get(col)) else ''
                        if old_val != new_val:
                            changed_fields.append(col)
                    
                    if changed_fields:
                        task_titre = edited_row.get('Titre', 'Tâche')
                        modifications.append(f"{task_titre}: {', '.join(changed_fields)}")
        ...
        # Forcer la synchronisation complète de la base de données sur le disque
        import sqlite3
        import time
        import os
        
        # Fermer toutes les connexions et forcer l'écriture sur disque
        conn = sqlite3.connect(db.DATABASE_FILE)
        # Forcer un checkpoint WAL pour écrire toutes les données sur le fichier principal
        conn.execute('PRAGMA wal_checkpoint(TRUNCATE)')
        conn.commit()
        conn.close()
        
        # Attendre que le système d'exploitation synchronise le fichier sur le disque
        time.sleep(1.0)
        ...
        # Mettre à jour le message : sauvegarde terminée avec détails
        if modifications:
            details = "\n".join([f"• {mod}" for mod in modifications[:3]])
            if len(modifications) > 3:
                details += f"\n• ... et {len(modifications) - 3} autre(s)"
            final_message = f"✅ Sauvegarde terminée !\n\n**Modifications :**\n{details}"
            if git_success:
                final_message += f"\n\n{git_message}"
            else:
                final_message += f"\n\n⚠️ {git_message}"
            status_placeholder.success(final_message, icon="✅")
        else:
            if git_success:
                status_placeholder.success(f"✅ Sauvegarde terminée !\n\n{git_message}", icon="✅")
            else:
                status_placeholder.warning(f"✅ Sauvegarde locale terminée\n\n⚠️ {git_message}", icon="⚠️")
        
        # Faire disparaître le message après 2 secondes pour permettre de continuer à modifier
        time.sleep(2.0)
...
def edit_task_dialog(task_id):
    ...
    # Bouton pour définir la date à +1 an (en dehors du formulaire)
    col_btn1, col_btn2 = st.columns([3, 1])
    with col_btn2:
        if st.button("📅 +1 an", key=f"deadline_preset_edit_{task_id}", use_container_width=True, help="Définir la date d'échéance à 1 an d'aujourd'hui"):
            from datetime import datetime, timedelta
            st.session_state[f'deadline_preset_edit_{task_id}'] = datetime.now().date() + timedelta(days=365)
            st.rerun()
    ...
    with st.form("edit_task_form"):
        ...
        # Catégorie dynamique basée sur la personne assignée
        available_categories = team_categories.get(assigned_to, [])
        current_category = task.get('category') if task.get('category') and pd.notna(task.get('category')) else None
        category_options_with_emoji = ["Aucune"] + get_category_options_with_emojis(available_categories)
        category_index = 0
        if current_category and current_category in available_categories:
            category_index = category_options_with_emoji.index(add_emoji_to_category(current_category))
        category = st.selectbox("Catégorie", category_options_with_emoji, index=category_index, help="Sélectionnez une catégorie pour cette tâche")
        category_value = None if category == "Aucune" else strip_emoji_from_value(category)
        
        # Date d'échéance avec calendrier
        from datetime import datetime as dt, date
        default_deadline_value = st.session_state.get(f'deadline_preset_edit_{task_id}', None)
        if default_deadline_value is None and task.get('deadline'):
            try:
                deadline_str = str(task['deadline']).strip()
                if deadline_str and deadline_str != '' and deadline_str.lower() != 'none':
                    default_deadline_value = dt.strptime(deadline_str, '%Y-%m-%d').date()
                else:
                    default_deadline_value = None
            except:
                default_deadline_value = None
        
        # Vérifier que la valeur est bien un objet date
        if default_deadline_value is not None and isinstance(default_deadline_value, date):
            deadline = st.date_input("Date d'échéance", value=default_deadline_value, help="Date limite pour cette tâche")
        else:
            deadline = st.date_input("Date d'échéance", help="Date limite pour cette tâche")
        ...
        if submitted:
            if title and assigned_to:
                ...
                db.update_task(task_id, title, description, client_name, assigned_to, priority, status, st.session_state.current_user, deadline_str, category_value)
                # Nettoyer le preset après mise à jour
                if f'deadline_preset_edit_{task_id}' in st.session_state:
                    del st.session_state[f'deadline_preset_edit_{task_id}']
...
def show_tasks_table_dialog(person_name, tasks_df):
    ...
    if not tasks_df.empty:
        ...
        # Nettoyer les emojis des colonnes pour permettre un filtrage correct
        if 'status' in tasks_df.columns:
            tasks_df['status'] = tasks_df['status'].apply(strip_emoji_from_value)
        if 'priority' in tasks_df.columns:
            tasks_df['priority'] = tasks_df['priority'].apply(strip_emoji_from_value)
        if 'category' in tasks_df.columns:
            tasks_df['category'] = tasks_df['category'].apply(strip_emoji_from_value)
        
        # Ajouter les numéros de tâche par personne
        tasks_df = add_person_task_numbers(tasks_df)
        
        # Obtenir toutes les catégories pour cette personne
        person_categories = team_categories.get(person_name, [])
        ...
        with col3:
            filter_category = st.multiselect(
                ...
                ["Sans catégorie"] + person_categories,
                default=["Sans catégorie"] + person_categories,
                key="filter_table_category",
                label_visibility="visible"
            )
        
        with col4:
            all_columns = ['Ordre', 'ID', 'Titre', 'Client', 'Description', 'Catégorie', 'Priorité', 'Statut', 'Date d\'échéance', 'Créée le', 'Modifiée le', 'Modifié par', 'Assigné à']
            selected_columns = st.multiselect(
                "📋 Colonnes à afficher",
                all_columns,
                default=['Ordre', 'Titre', 'Catégorie', 'Priorité', 'Statut', 'Date d\'échéance', 'Assigné à'],
        ...
        if "Sans catégorie" in filter_category:
            # Inclure les tâches sans catégorie et celles avec catégories sélectionnées
            categories_to_filter = [cat for cat in filter_category if cat != "Sans catégorie"]
            if categories_to_filter:
                filtered_df = filtered_df[(filtered_df['category'].isin(categories_to_filter)) | (filtered_df['category'].isna())]
            else:
                # Seulement les tâches sans catégorie
                filtered_df = filtered_df[filtered_df['category'].isna()]
        else:
            # Seulement les tâches avec catégories sélectionnées
            filtered_df = filtered_df[filtered_df['category'].isin(filter_category)]
        
        # Filtre par date d'échéance
        # Vérifier que les valeurs sont définies (pas None)
        has_start_date = filter_date_start is not None
        has_end_date = filter_date_end is not None
        
        if has_start_date or has_end_date:
            # Convertir la colonne deadline en datetime pour la comparaison
            filtered_df['deadline_dt'] = pd.to_datetime(filtered_df['deadline'], errors='coerce')
            
            if has_start_date:
                filter_date_start_dt = pd.to_datetime(filter_date_start)
                filtered_df = filtered_df[
                    (filtered_df['deadline_dt'].isna()) | (filtered_df['deadline_dt'] >= filter_date_start_dt)
                ]
            
            if has_end_date:
                filter_date_end_dt = pd.to_datetime(filter_date_end)
                filtered_df = filtered_df[
                    (filtered_df['deadline_dt'].isna()) | (filtered_df['deadline_dt'] <= filter_date_end_dt)
                ]
            
            # Supprimer la colonne temporaire
            filtered_df = filtered_df.drop('deadline_dt', axis=1)
        
        # Détecter si des filtres sont actifs
        all_statuses = ["À faire", "En cours", "Terminée"]
        all_priorities = ["Normale", "Urgente"]
        all_categories_with_none = ["Sans catégorie"] + person_categories
        has_active_filters = (set(filter_status) != set(all_statuses) or set(filter_priority) != set(all_priorities) or set(filter_category) != set(all_categories_with_none) or has_start_date or has_end_date)
        ...
        if filtered_df.empty:
            ...
            # Convertir la colonne deadline en format date AVANT de renommer
            ...
            # Remplir les valeurs nulles pour toutes les colonnes sauf les dates
            for col in display_df.columns:
                if col != 'Date d\'échéance':
                    display_df[col] = display_df[col].fillna('')
            
            column_config = {}
            if 'Ordre' in selected_columns:
                column_config["Ordre"] = st.column_config.NumberColumn("Ordre", width=50, help="Modifiez pour réorganiser les tâches", required=True, min_value=1)
            if 'ID' in selected_columns:
                column_config["ID"] = st.column_config.NumberColumn("ID", width=50, disabled=True)
            if 'Titre' in selected_columns:
                column_config["Titre"] = st.column_config.TextColumn("Titre", width=200, required=True)
            if 'Client' in selected_columns:
                column_config["Client"] = st.column_config.TextColumn("Client", width=150)
            if 'Description' in selected_columns:
                column_config["Description"] = st.column_config.TextColumn("Description", width=1000)
            if 'Catégorie' in selected_columns:
                column_config["Catégorie"] = st.column_config.SelectboxColumn("Catégorie", width=150, options=[""] + person_categories, help="Catégorie de la tâche")
            if 'Priorité' in selected_columns:
                column_config["Priorité"] = st.column_config.SelectboxColumn("Priorité", width=90, options=["🟢 Normale", "🔴 Urgente"], required=True)
            if 'Statut' in selected_columns:
                column_config["Statut"] = st.column_config.SelectboxColumn("Statut", width=90, options=["À faire", "En cours", "Terminée"], required=True)
            if 'Date d\'échéance' in selected_columns:
                column_config["Date d'échéance"] = st.column_config.DateColumn("Date d'échéance", width=150, format="YYYY-MM-DD", help="Cliquez pour ouvrir le calendrier")
            if 'Créée le' in selected_columns:
                column_config["Créée le"] = st.column_config.TextColumn("Créée le", width=120, disabled=True)
            if 'Modifiée le' in selected_columns:
                column_config["Modifiée le"] = st.column_config.TextColumn("Modifiée le", width=120, disabled=True)
            if 'Modifié par' in selected_columns:
                column_config["Modifié par"] = st.column_config.TextColumn("Modifié par", width=100, disabled=True)
            if 'Assigné à' in selected_columns:
                column_config["Assigné à"] = st.column_config.SelectboxColumn("Assigné à", width=100, options=team_members, required=True)
...
if not st.session_state.user_logged_in:
...
elif st.session_state.user_logged_in and st.session_state.app_mode is None:
    st.markdown("""
    ...
    # Filtre pour les tâches urgentes : priorité "Urgente" OU (Lucas ET catégorie "INFO URGENTE")
    if not user_tasks.empty:
        urgent_by_priority = user_tasks['priority'] == 'Urgente'
        
        # Ajouter les tâches de Lucas avec catégorie "INFO URGENTE"
        if current_user == 'Lucas' and 'category' in user_tasks.columns:
            info_urgente = user_tasks['category'] == 'INFO URGENTE'
            urgent_tasks = user_tasks[urgent_by_priority | info_urgente]
        else:
            urgent_tasks = user_tasks[urgent_by_priority]
    else:
    ...
    with tab1:
        ...
        if urgent_tasks.empty:
            ...
            # Nettoyer les emojis des colonnes pour permettre un filtrage correct
            if 'status' in filtered_urgent_tasks.columns:
                filtered_urgent_tasks['status'] = filtered_urgent_tasks['status'].apply(strip_emoji_from_value)
            if 'priority' in filtered_urgent_tasks.columns:
                filtered_urgent_tasks['priority'] = filtered_urgent_tasks['priority'].apply(strip_emoji_from_value)
            if 'category' in filtered_urgent_tasks.columns:
                filtered_urgent_tasks['category'] = filtered_urgent_tasks['category'].apply(strip_emoji_from_value)
            ...
            # Trier pour mettre les tâches "INFO URGENTE" de Lucas en premier
            if not filtered_urgent_tasks.empty:
                def get_sort_priority(row):
                    # Priorité 0 : Tâches de Lucas avec catégorie "INFO URGENTE"
                    assigned = row['assigned_to'] if 'assigned_to' in row.index else ''
                    category = row['category'] if 'category' in row.index else ''
                    
                    # Nettoyer la catégorie pour être sûr (enlever emoji si présent)
                    if pd.notna(category):
                        category = str(category).replace('🚨 ', '').replace('🚨', '').strip()
                    
                    if assigned == 'Lucas' and category == 'INFO URGENTE':
                        return 0
                    # Priorité 1 : Toutes les autres tâches urgentes
                    return 1
                
                filtered_urgent_tasks['_sort_priority'] = filtered_urgent_tasks.apply(get_sort_priority, axis=1)
                filtered_urgent_tasks = filtered_urgent_tasks.sort_values(by='_sort_priority').drop(columns=['_sort_priority'])
            ...
            if filtered_urgent_tasks.empty:
                ...
                # Afficher les tâches en format carte
                for _, task in filtered_urgent_tasks.iterrows():
                    ...
                    with col1:
                        ...
                        # Description comme titre principal
                        description_text = task.get('description') if task.get('description') and pd.notna(task.get('description')) and str(task.get('description')).strip() else "Aucune description"
                        html_parts.append(f'<h2 style="margin: 0 0 6px 0; font-size: 18px; font-weight: 600; color: #E8E9ED;">{escape_html(description_text)}</h2>')
                        html_parts.append('<div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">')
                        
                        # Client badge (qui a donné la tâche)
                        if task.get('client_name') and pd.notna(task.get('client_name')) and str(task.get('client_name')).strip():
                        ...
                        # Status, catégorie et date d'échéance
                        ...
                        # Catégorie
                        if task.get('category') and pd.notna(task.get('category')) and str(task.get('category')).strip():
                            category_emoji = CATEGORY_EMOJI_MAP.get(str(task['category']), '⚪')
                            html_parts.append(f'<span class="subtitle" style="white-space: nowrap; font-size: 11px;">{category_emoji} {escape_html(task["category"])}</span>')
                        
                        # Date d'échéance
                        if task.get('deadline') and pd.notna(task.get('deadline')):
                            html_parts.append(f'<span class="subtitle" style="white-space: nowrap; font-size: 11px;">📅 {escape_html(str(task["deadline"]))}</span>')
                ...
                st.markdown("<br>", unsafe_allow_html=True)
                st.markdown(f"**Total:** {len(filtered_urgent_tasks)} tâche(s) urgente(s)")
    ...
    with tab2:
        ...
        # Section "Assigner à" avec design iOS premium
        st.markdown("""
            <div class="assign-section-container">
                <div class="assign-section-header">
                    <div class="assign-icon">👤</div>
                    <div class="assign-text">
                        <h3 style="margin: 0; font-size: 16px; font-weight: 600; color: #E8E9ED;">Assigner à</h3>
                        <p style="margin: 0; font-size: 13px; color: #9CA3AF;">Choisissez un membre de l'équipe</p>
                    </div>
                </div>
            </div>
        """, unsafe_allow_html=True)
        
        col_assign, col_date = st.columns([2, 1])
        
        with col_assign:
            assigned_to = st.selectbox("Assigner à *", team_members, index=0, key="new_task_assigned_to", label_visibility="collapsed")
        
        with col_date:
            if st.button("📅 +1 an", key="btn_deadline_preset", use_container_width=True, help="Définir la date d'échéance à 1 an d'aujourd'hui"):
                from datetime import datetime, timedelta
                st.session_state.deadline_preset = datetime.now().date() + timedelta(days=365)
        
        # Catégories disponibles basées sur la personne sélectionnée
        available_categories = team_categories.get(assigned_to, [])
        ...
        with st.form("new_task_form", clear_on_submit=True):
            ...
            with col2:
                category = st.selectbox("Catégorie", ["Aucune"] + get_category_options_with_emojis(available_categories), index=0, help="Sélectionnez une catégorie pour cette tâche")
            
            category_value = None if category == "Aucune" else strip_emoji_from_value(category)
            
            # Date d'échéance toujours visible avec valeur par défaut
            default_deadline = st.session_state.get('deadline_preset', None)
            if default_deadline is not None:
                deadline = st.date_input("Date d'échéance *", value=default_deadline, help="Date limite pour cette tâche")
            else:
                deadline = st.date_input("Date d'échéance *", help="Date limite pour cette tâche")
            ...
            if submitted:
                if title and assigned_to:
                    deadline_str = deadline.strftime('%Y-%m-%d') if deadline else None
                    db.add_task(title, description, assigned_to, priority, client_name, deadline_str, category_value)
                    # Réinitialiser le preset après ajout
    ...
    with tab3:
        ...
        if person_tasks.empty:
            ...
            # Obtenir les catégories pour cette personne
            person_categories = team_categories.get(selected_person, [])
            
            # Section Filtres avec design iOS premium
            st.markdown("""
                <div class="filters-container">
                    <div class="filters-header">
                        <div class="filters-icon">🔍</div>
                        <h3 style="margin: 0; font-size: 18px; font-weight: 600; color: #E8E9ED;">Filtres et Recherche</h3>
                    </div>
                </div>
            """, unsafe_allow_html=True)
            
            # Ligne 1: Filtres multiselect
            col1, col2, col3, col4 = st.columns(4)
            ...
            with col4:
                ...
                selected_columns = st.multiselect(
                    "📋 Colonnes à afficher",
                    all_columns,
                    default=['Ordre', 'Priorité', 'Statut', 'Description', 'Client', 'Catégorie', 'Date d\'échéance'],
                    key="filter_person_columns"
                )
            
            # Ligne 2: Champ de recherche client
            st.markdown('<div style="margin-top: 12px;"></div>', unsafe_allow_html=True)
            search_client = st.text_input(
                "🔎 Rechercher un client",
                placeholder="Entrez le nom d'un client...",
                key="search_person_client",
                label_visibility="visible"
            )
            ...
            # Nettoyer les emojis des colonnes pour permettre un filtrage correct
            if 'status' in filtered_person_tasks.columns:
                filtered_person_tasks['status'] = filtered_person_tasks['status'].apply(strip_emoji_from_value)
            if 'priority' in filtered_person_tasks.columns:
                filtered_person_tasks['priority'] = filtered_person_tasks['priority'].apply(strip_emoji_from_value)
            if 'category' in filtered_person_tasks.columns:
                filtered_person_tasks['category'] = filtered_person_tasks['category'].apply(strip_emoji_from_value)
            ...
            if search_client:
                filtered_person_tasks = filtered_person_tasks[
                    filtered_person_tasks['client_name'].str.contains(search_client, case=False, na=False)
                ]
            ...
            if "Sans catégorie" in filter_category:
                categories_to_filter = [cat for cat in filter_category if cat != "Sans catégorie"]
                if categories_to_filter:
                    filtered_person_tasks = filtered_person_tasks[(filtered_person_tasks['category'].isin(categories_to_filter)) | (filtered_person_tasks['category'].isna())]
                else:
                    filtered_person_tasks = filtered_person_tasks[filtered_person_tasks['category'].isna()]
            else:
                filtered_person_tasks = filtered_person_tasks[filtered_person_tasks['category'].isin(filter_category)]
            
            # Détecter si des filtres sont actifs
            all_statuses = ["À faire", "En cours", "Terminée"]
            all_priorities = ["Normale", "Urgente"]
            all_categories_with_none = ["Sans catégorie"] + person_categories
            has_active_filters = (set(filter_status) != set(all_statuses) or set(filter_priority) != set(all_priorities) or set(filter_category) != set(all_categories_with_none))
            ...
            if filtered_person_tasks.empty:
                ...
                if view_mode == "🔀 Réorganiser":
                    ...
                    for idx, task in enumerate(tasks_list):
                        ...
                        with col_num:
                            ...
                            if new_position != idx + 1:
                                ...
                                # Enregistrer automatiquement l'ordre
                                new_order_map = {}
                        ...
                        with col_up:
                            if idx > 0:
                                if st.button("↑", key=f"up_{idx}_{task_id}", help="Monter"):
                                    ...
                                    new_order_map = {}
                                    for new_idx, task in enumerate(tasks_list):
                                        task_id_save = int(task['_original_id'])
                                        new_order_map[task_id_save] = new_idx + 1
                                    db.reorder_tasks(selected_person, new_order_map)
                    ...
                    st.info("💾 Les modifications sont enregistrées automatiquement")
                
                else:
                    # Mode Tableau avec boutons d'édition
                    st.markdown("<br>", unsafe_allow_html=True)
                    ...
                    # Ajouter la colonne Ordre (toujours disponible)
                    display_df['Ordre'] = display_df['person_task_number']
                    ...
                    # Afficher le tableau avec boutons d'édition
                    # Passer les catégories de la personne sélectionnée
                    person_categories = team_categories.get(selected_person, [])
                    render_tasks_table_with_edit(
                        display_df, 
                        original_df, 
                        column_order,
                        person_categories
                    ...
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown(f"**Total:** {len(display_df)} tâche(s)")
                    st.markdown("💡 Cliquez sur ✏️ pour modifier une tâche")
    ...
    with tab4:
        ...
        if all_tasks.empty:
            ...
            # Créer une copie et nettoyer les emojis
            tasks_for_filtering = all_tasks.copy()
            if 'status' in tasks_for_filtering.columns:
                tasks_for_filtering['status'] = tasks_for_filtering['status'].apply(strip_emoji_from_value)
            if 'priority' in tasks_for_filtering.columns:
                tasks_for_filtering['priority'] = tasks_for_filtering['priority'].apply(strip_emoji_from_value)
            if 'category' in tasks_for_filtering.columns:
                tasks_for_filtering['category'] = tasks_for_filtering['category'].apply(strip_emoji_from_value)
            
            filtered_tasks = tasks_for_filtering[
                (tasks_for_filtering['assigned_to'].isin(filter_person)) &
                (tasks_for_filtering['status'].isin(filter_status))
            ]
...
with st.sidebar:
    ...
    # Section GitHub Sync
    if github_backup.is_github_configured():
        st.markdown("""
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                ...
                <h3 style="color: white; margin: 0 0 4px 0; font-size: 16px;">
                    🔄 GitHub Sync
                </h3>
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📤 Upload", use_container_width=True, help="Forcer upload manuel"):
                with st.spinner("Upload en cours..."):
                    success, message = auto_save_to_github()
                    if success:
                        st.success(message)
                    else:
                        st.error(message)
        ...
        with col2:
            if st.button("📥 Download", use_container_width=True, help="Restaurer depuis GitHub"):
                with st.spinner("Download en cours..."):
                    success, message = github_backup.download_from_github(db.DATABASE_FILE)
                    if success:
                        st.success(message)
                        st.rerun()
                    else:
        ...
        st.caption("💡 Upload automatique à chaque modification")
    else:
        st.info("""
        **🔄 GitHub Sync non configuré**
        
        Pour activer la synchronisation automatique, configurez vos credentials GitHub.
        """)
        
        if st.button("⚙️ Configurer GitHub", use_container_width=True):
            st.info("""
